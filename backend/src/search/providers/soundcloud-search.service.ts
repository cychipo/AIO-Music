...
export class SoundcloudSearchService implements OnModuleInit {
  ...
  private async _refreshClientId(): Promise<void> {
    try {
      ...
      this.logger.log(`[SoundCloud Search] client_id refreshed: ${id.slice(0, 8)}...`);
    } catch (err) {
      this.logger.warn(`[SoundCloud Search] Failed to refresh client_id: ${err.message}`);
    }
  }
  ...
  async search(query: string, limit = 10): Promise<SearchResult[]> {
    try {
      ...
      this.logger.warn(`[SoundCloud Search] Failed, refreshing client_id: ${err.message}`);
      await this._refreshClientId();
  ...
  private _mapResults(results: any[]): SearchResult[] {
