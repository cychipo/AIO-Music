...
describe('AuthService', () => {
  ...
  describe('register', () => {
    ...
    it('nên ném ConflictException khi email đã tồn tại (bất kể provider)', async () => {
      ...
      await expect(
        service.register({ email: 'test@example.com', password: 'pass123', displayName: 'X' }),
    ...
    it('nên ném ConflictException khi email thuộc OAuth account', async () => {
      ...
      userModel.findOne.mockResolvedValue(makeUserDoc({ authProvider: 'google', password: null }));
    ...
    it('nên hash password trước khi lưu', async () => {
      ...
      await service.register({ email: 'test@example.com', password: 'plaintext123', displayName: 'T' });
    ...
    it('nên set authProvider = local khi register bằng email', async () => {
      ...
      await service.register({ email: 'test@example.com', password: 'pass123', displayName: 'T' });
  ...
  describe('login', () => {
    ...
    it('nên ném UnauthorizedException với message gợi ý provider khi user đăng ký bằng OAuth', async () => {
      ...
      userModel.findOne.mockResolvedValue(makeUserDoc({ password: null, authProvider: 'google' }));
    ...
    it('nên ném UnauthorizedException khi tài khoản bị vô hiệu hoá', async () => {
      const hash = await bcrypt.hash('password123', 10);
      userModel.findOne.mockResolvedValue(makeUserDoc({ password: hash, isActive: false }));
  ...
  describe('findOrCreateOAuthUser', () => {
    it('nên trả về user hiện có khi tìm thấy theo providerId', async () => {
      const existingUser = makeUserDoc({ authProvider: 'google', providerId: 'google-sub-123' });
    ...
    it('nên trả về user hiện có và link providerId khi email khớp nhưng chưa có providerId', async () => {
      const localUser = makeUserDoc({ authProvider: 'local', providerId: null });
    ...
    it('nên tạo tài khoản mới với authProvider=google khi email chưa tồn tại', async () => {
      userModel.findOne.mockResolvedValue(null);
      const newUser = makeUserDoc({ authProvider: 'google', providerId: 'google-sub-123', password: null });
    ...
    it('nên ném UnauthorizedException khi user bị vô hiệu hoá (tìm theo providerId)', async () => {
      userModel.findOne.mockResolvedValueOnce(
        makeUserDoc({ authProvider: 'google', providerId: 'google-sub-123', isActive: false }),
      );

      await expect(service.findOrCreateOAuthUser(googleProfile)).rejects.toThrow(UnauthorizedException);
    });
    ...
    it('nên normalize email về lowercase khi tạo OAuth user', async () => {
      ...
      await service.findOrCreateOAuthUser({ ...googleProfile, email: 'Google@EXAMPLE.COM' });
  ...
  describe('refresh', () => {
    ...
    it('nên ném UnauthorizedException khi token đã bị revoke', async () => {
      refreshTokenModel.findOne.mockResolvedValue(makeRefreshTokenDoc({ revoked: true }));
  ...
  describe('validateById', () => {
    ...
    it('nên ném UnauthorizedException khi user không tồn tại', async () => {
      ...
      await expect(service.validateById('bad-id')).rejects.toThrow(UnauthorizedException);
    });
    ...
    it('nên ném UnauthorizedException khi user bị deactivate', async () => {
      ...
      await expect(service.validateById('user-id-123')).rejects.toThrow(UnauthorizedException);
    });
  });
});
